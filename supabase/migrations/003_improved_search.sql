-- ============================================================
-- Noteworthy — Improved search
-- Migration: 003_improved_search.sql
--
-- Fixes:
-- 1. Content search now extracts actual text from Tiptap JSONB
--    instead of searching raw JSON markup.
-- 2. Adds ILIKE fallback so partial words match instantly
--    (e.g. typing "lov" finds "loving").
-- ============================================================

-- Helper: recursively extract plain text from a Tiptap JSONB document.
-- Uses jsonb_path_query to find all {"type":"text","text":"..."} nodes.
...
AS $$
  ...
  FROM jsonb_path_query(doc, '$.** ? (@.type == "text")') AS elem;
$$;

-- Replace the search function with the improved version.
...
BEGIN
  ...
  WHERE j.user_id    = auth.uid()
    ...
    -- Match via full-text search (stemmed words) OR substring ILIKE (partial words)
    AND (
